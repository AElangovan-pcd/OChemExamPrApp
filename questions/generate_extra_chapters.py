...
compounds_db = [
    ...
    {"name": "Formic acid", "smiles": "C(=O)O", "formula": "CH2O2", "mw": 46, "signals": [{"x": 8.0, "y": 1, "label": "singlet"}, {"x": 11.0, "y": 1, "label": "singlet"}], "base_peak": 29, "parent_peak": 46, "dips": [{"xc": 3000, "w": 300, "d": 75}, {"xc": 1710, "w": 20, "d": 85}]},
...
amino_acids = [
    ...
    {"name": "Cysteine", "smiles": "SC[C@@H](C(=O)O)N"},
...
sugars = [
    ...
    {"name": "D-Mannose", "smiles": "OC[C@@H](O)[C@@H](O)[C@H](O)[C@H](O)C=O"},
...
def generate_all_extra_chapters():
    ...
    for ch in range(33, 51):
        ...
        for q_idx in range(1, 96):
            ...
            # -------------------------------------------------------------
            # Chapter 49: Biomolecules - Lipids MCQs & Matching
            # -------------------------------------------------------------
            elif ch == 49:
                # We will have 5 question templates
                q_type = q_idx % 5
                
                if q_type == 0:
                    # Fatty acid melting point
                    options = [
                        {"option_id": "A", "text": "Oleic acid, because the cis double bond introduces a rigid bend that prevents efficient packing.", "is_correct": True},
                        {"option_id": "B", "text": "Stearic acid, because saturated chains pack less efficiently than unsaturated ones.", "is_correct": False},
                        {"option_id": "C", "text": "Oleic acid, because it has a higher molecular weight than stearic acid.", "is_correct": False},
                        {"option_id": "D", "text": "Stearic acid, because the trans double bond increases packing density.", "is_correct": False}
                    ]
                    random.shuffle(options)
                    for idx_opt, opt in enumerate(options):
                        opt["option_id"] = chr(65 + idx_opt)
                    ...
                    terpenes = [
                        {"name": "Menthol", "smiles": "CC1CCC(C(C1)O)C(C)C", "type": "monoprene", "units": 2, "class": "monoterpene (10 carbons)"},
                        {"name": "Limonene", "smiles": "CC1=CCC(CC1)C(=C)C", "type": "monoprene", "units": 2, "class": "monoterpene (10 carbons)"},
                        {"name": "Squalene", "smiles": "CC(C)=CCCC(C)=CCCC(C)=CCCC=C(C)CCC=C(C)CCC=C(C)C", "type": "triterpene", "units": 6, "class": "triterpene (30 carbons)"}
                    ]
                    t = terpenes[q_idx % len(terpenes)]
                    
                    options = [
                        {"option_id": "A", "text": f"{t['units']} units, classifying it as a {t['class']}.", "is_correct": True},
                        {"option_id": "B", "text": f"{t['units'] - 1} units, classifying it as a sesquiterpene.", "is_correct": False},
                        {"option_id": "C", "text": f"{t['units'] + 1} units, classifying it as a diterpene.", "is_correct": False},
                        {"option_id": "D", "text": f"{t['units'] * 2} units, classifying it as a polyterpene.", "is_correct": False}
                    ]
                    random.shuffle(options)
                    for idx_opt, opt in enumerate(options):
                        opt["option_id"] = chr(65 + idx_opt)
                    ...
                    questions.append({
                        ...
                        "topic": "Biomolecules: Lipids",
                        "difficulty_level": "Hard",
                        "question_text": f"Analyze the terpene molecule {t['name']}. Apply the isoprene rule to identify how many isoprene units make up its skeleton and determine its classification.",
                        "question_smiles": t["smiles"],
                        "options": options,
                        "feedback": {
                            "context": "Applying the isoprene rule to classify terpenes.",
                            "process": f"Step 1: Skeletons of terpenes are built from isoprene (2-methyl-1,3-butadiene, C5) units connected in a head-to-tail fashion.\\nStep 2: Divide the total carbon count of {t['name']} by 5 to find the number of units: it has {t['units']} isoprene units.\\nStep 3: Terpenes with 10 carbons (2 units) are monoterpenes, while 30 carbons (6 units) are triterpenes.",
                            "result": f"{t['name']} is composed of {t['units']} isoprene units."
                        }
                    })
                elif q_type == 2:
                    # Saponification reaction
                    options = [
                        {"option_id": "A", "text": "Glycerol and sodium stearate (soap)", "smiles": "OCC(O)CO.[Na+].[O-]C(=O)CCCCCCCCCCCCCCCCC", "is_correct": True},
                        {"option_id": "B", "text": "Stearic acid and sodium hydroxide", "smiles": "CCCCCCCCCCCCCCCCCC(=O)O.[Na+].[OH-]", "is_correct": False},
                        {"option_id": "C", "text": "Ethyl stearate and glycerol", "smiles": "CCOC(=O)CC(O)C.OCC(O)CO", "is_correct": False},
                        {"option_id": "D", "text": "Adipic acid and sodium propanoate", "smiles": "OC(=O)CCCCC(=O)O.CCC(=O)O[Na]", "is_correct": False}
                    ]
                    random.shuffle(options)
                    for idx_opt, opt in enumerate(options):
                        opt["option_id"] = chr(65 + idx_opt)
                    ...
                    options = [
                        {"option_id": "A", "text": "A fused tetracyclic ring system consisting of three cyclohexane rings and one cyclopentane ring.", "is_correct": True},
                        {"option_id": "B", "text": "A fused tricyclic ring system containing three cyclohexane rings.", "is_correct": False},
                        {"option_id": "C", "text": "Four cyclohexane rings fused linearly in a row.", "is_correct": False},
                        {"option_id": "D", "text": "A macrocyclic ring of 17 carbon atoms containing various heteroatoms.", "is_correct": False}
                    ]
                    random.shuffle(options)
                    for idx_opt, opt in enumerate(options):
                        opt["option_id"] = chr(65 + idx_opt)
                    ...
                    options = [
                        {"option_id": "A", "text": "Phosphoglyceride (lecithin), which forms lipid bilayers due to its amphipathic nature.", "is_correct": True},
                        {"option_id": "B", "text": "Triacylglycerol, which forms micelles to store energy in adipose tissue.", "is_correct": False},
                        {"option_id": "C", "text": "Prostaglandin, which acts as a hydrophobic signalling lipid.", "is_correct": False},
                        {"option_id": "D", "text": "Sphingomyelin, which contains an ester-linked glycerol backbone.", "is_correct": False}
                    ]
                    random.shuffle(options)
                    for idx_opt, opt in enumerate(options):
                        opt["option_id"] = chr(65 + idx_opt)
            ...
            # -------------------------------------------------------------
            # Chapter 50: Biomolecules - Nucleic Acids MCQs
            # -------------------------------------------------------------
            elif ch == 50:
                q_type = q_idx % 5
                
                if q_type == 0:
                    # Hydrogen bonding counts
                    options = [
                        {"option_id": "A", "text": "Adenine pairs with Thymine via 2 hydrogen bonds; Guanine pairs with Cytosine via 3 hydrogen bonds.", "is_correct": True},
                        {"option_id": "B", "text": "Adenine pairs with Cytosine via 2 hydrogen bonds; Guanine pairs with Thymine via 3 hydrogen bonds.", "is_correct": False},
                        {"option_id": "C", "text": "Adenine pairs with Thymine via 3 hydrogen bonds; Guanine pairs with Cytosine via 2 hydrogen bonds.", "is_correct": False},
                        {"option_id": "D", "text": "Adenine pairs with Uracil via 3 hydrogen bonds in double-stranded DNA.", "is_correct": False}
                    ]
                    random.shuffle(options)
                    for idx_opt, opt in enumerate(options):
                        opt["option_id"] = chr(65 + idx_opt)
                    ...
                    options = [
                        {"option_id": "A", "text": "AMP is a nucleotide because it contains a phosphate group esterified at C5' of the ribose ring.", "is_correct": True},
                        {"option_id": "B", "text": "Adenosine is a nucleotide because it contains a purine base linked to a ribose ring.", "is_correct": False},
                        {"option_id": "C", "text": "AMP is a nucleoside because it has no phosphate group.", "is_correct": False},
                        {"option_id": "D", "text": "Adenosine is a nucleotide because it is phosphorylated.", "is_correct": False}
                    ]
                    random.shuffle(options)
                    for idx_opt, opt in enumerate(options):
                        opt["option_id"] = chr(65 + idx_opt)
                    ...
                    options = [
                        {"option_id": "A", "text": "5'-G-C-T-A-C-G-T-3'", "is_correct": True},
                        {"option_id": "B", "text": "5'-T-G-C-A-T-C-G-3'", "is_correct": False},
                        {"option_id": "C", "text": "5'-A-C-G-T-A-G-C-3'", "is_correct": False},
                        {"option_id": "D", "text": "5'-C-G-T-A-C-G-T-3'", "is_correct": False}
                    ]
                    random.shuffle(options)
                    for idx_opt, opt in enumerate(options):
                        opt["option_id"] = chr(65 + idx_opt)
                    ...
                    # Sanger sequencing
                    options = [
                        {"option_id": "A", "text": "Dideoxynucleotides (ddNTPs) lack the 3'-OH group, preventing further phosphodiester bond formation and terminating the chain.", "is_correct": True},
                        {"option_id": "B", "text": "Dideoxynucleotides lack the 2'-OH group, converting DNA into RNA.", "is_correct": False},
                        {"option_id": "C", "text": "Dideoxynucleotides lack the nitrogenous base, creating abasic sites.", "is_correct": False},
                        {"option_id": "D", "text": "Dideoxynucleotides carry a triphosphate group on the 3' position.", "is_correct": False}
                    ]
                    random.shuffle(options)
                    for idx_opt, opt in enumerate(options):
                        opt["option_id"] = chr(65 + idx_opt)
                    ...
                    options = [
                        {"option_id": "A", "text": "Acid-catalyzed cleavage of the dimethoxytrityl (DMT) ether protecting group on C5' using trichloroacetic acid.", "is_correct": True},
                        {"option_id": "B", "text": "Deprotection of the cyanoethyl protecting group using liquid ammonia.", "is_correct": False},
                        {"option_id": "C", "text": "Activation of the phosphoramidite phosphorus atom using a strong Lewis base.", "is_correct": False},
                        {"option_id": "D", "text": "Oxidation of the phosphite triester intermediate using sulfur dioxide.", "is_correct": False}
                    ]
                    random.shuffle(options)
                    for idx_opt, opt in enumerate(options):
                        opt["option_id"] = chr(65 + idx_opt)
